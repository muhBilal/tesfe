...
export default function page({params}) {
  const [artefafcts, setArtefafcts] = useState([]);
  const [artefactData, setArtefactData] = useState([]);
  const [shuffle, setShuffle] = useState([]);

  const fetchData = async () => {
    const options = {year: 'numeric', month: 'long', day: 'numeric'}
    const res = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/artefacts');
    setArtefafcts(res.data.data);

    const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/artefacts/' + params.id);
  ...
  const shuffleData = async (musics) => {
    if (musics) {
      const shuffled = [...musics]; // Create a shallow copy of the musics array
  ...
  useEffect(() => {
    shuffleData(artefafcts)
  }, [artefafcts, artefactData]);
            ...
            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
              <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                <div>
                  <div className={`bg-white rounded-xl `}>
                    <img
                        src={process.env.NEXT_PUBLIC_STORAGE + artefactData.image_thumbnail}
                        alt=""
                    ...
                    >
                      <div className={`w-full pr-3 px-6`}>
                        <div className={`flex gap-5 items-center mb-3`}>
                          ...
                          <h1 className={`text-gray-400`}>{artefactData.createdAt}</h1>
                        </div>
                        ...
                        <div className={`text-gray-500 text-lg`}
                             dangerouslySetInnerHTML={{__html: artefactData.desc}}/>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className={`flex flex-col gap-5`}>
                  {shuffle.map((artefact) => (
                      <div>
                        <img
                            src={`${process.env.NEXT_PUBLIC_STORAGE}${artefact.image_thumbnail}`}
                            alt="animals"
                        ...
                        <div>
                          ...
                          <div className={`text-gray-500 text-sm`}
                               dangerouslySetInnerHTML={{__html: artefact.desc}}/>
                        </div>
                      </div>
