...
export default function page({params}) {
    ...
    const fetchData = async () => {
        ...
        const res = await axios.get(process.env.NEXT_PUBLIC_API + '/provinces/animals');
        setAnimals(res.data.data);

        const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/provinces/animals/' + params.id);
        const data = res_data.data.data;
        data.createdAt = new Date(data.created_at).toLocaleDateString('id-ID', options);
        setAnimalData(data)
    }

    const shuffleAnimals = async (animals) => {
        if (animals) {
            const shuffled = [...animals]; // Create a shallow copy of the animals array
                                    ...
                                    >
                                        <div className={`w-full pr-3 px-6`}>
                                            <div className={`flex gap-5 items-center mb-3`}>
                                                ...
                                                <h1 className={`text-gray-400`}>{animalData.createdAt}</h1>
                                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div>
                                            ...
                                            <div className={`text-gray-500 text-sm`}
                                                 dangerouslySetInnerHTML={{__html: animal.desc}}/>
                                        </div>
                                    </div>
