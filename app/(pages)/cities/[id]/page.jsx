...
export default function page({params}) {
  ...
  const fetchData = async () => {
    ...
    const res = await axios.get(process.env.NEXT_PUBLIC_API + '/cities');
    setCities(res.data.data);

    const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/' + params.id);
    const data = res_data.data.data;
    data.createdAt = new Date(data.created_at).toLocaleDateString('id-ID', options);
    setFoodData(data)
  }

  const shuffleData = async (cities) => {
    if (cities) {
      const shuffled = [...cities]; // Create a shallow copy of the cities array
              ...
              >
                ...
                <div className={`flex flex-col gap-5`}>
                  {shuffle.map((city) => (
                      <div>
                        <img
                            src={`${process.env.NEXT_PUBLIC_STORAGE}${city.image_thumbnail}`}
                            alt="animals"
                        ...
                        <div>
                          ...
                          <div className={`text-gray-500 text-sm`}
                               dangerouslySetInnerHTML={{__html: city.desc}}/>
                        </div>
                      </div>
