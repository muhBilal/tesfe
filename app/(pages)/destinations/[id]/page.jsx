...
export default function page({params}) {
  ...
  const fetchData = async () => {
    ...
    const res = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/destinations');
    setDestinations(res.data.data);

    const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/destinations/' + params.id);
            ...
            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
              <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                <div>
                  <div className={`bg-white rounded-xl `}>
                    <img
                        src={process.env.NEXT_PUBLIC_STORAGE + destinationData.image_thumbnail}
                        alt=""
                    ...
                    >
                      <div className={`w-full pr-3 px-6`}>
                        <div className={`flex gap-5 items-center mb-3`}>
                          ...
                          <h1 className={`text-gray-400`}>{destinationData.createdAt}</h1>
                        </div>
                        ...
                        <div className={`text-gray-500 text-lg`}
                             dangerouslySetInnerHTML={{__html: destinationData.desc}}/>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className={`flex flex-col gap-5`}>
                  {shuffle.map((destination) => (
                      <div>
                        <img
                            src={`${process.env.NEXT_PUBLIC_STORAGE}${destination.image_thumbnail}`}
                            alt="animals"
                        ...
                        <div>
                          ...
                          <div className={`text-gray-500 text-sm`}
                               dangerouslySetInnerHTML={{__html: destination.desc}}/>
                        </div>
                      </div>
