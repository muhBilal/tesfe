...
export default function page({params}) {
  const [vegetations, setVegetations] = useState([]);
  const [vegetationData, setVegetationData] = useState([]);
  const [shuffle, setShuffle] = useState([]);

  const fetchData = async () => {
    const options = {year: 'numeric', month: 'long', day: 'numeric'}
    const res = await axios.get(process.env.NEXT_PUBLIC_API + '/provinces/vegetations');
    setVegetations(res.data.data);

    const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/provinces/vegetations/' + params.id);
  ...
  const shuffleAnimals = async (vegetations) => {
    if (vegetations) {
      const shuffled = [...vegetations]; // Create a shallow copy of the vegetations array
  ...
  useEffect(() => {
    shuffleAnimals(vegetations)
  }, [vegetations, vegetationData]);
            ...
            <section className={`flex flex-col md:flex-row gap-5 mt-2`}>
              <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                <div>
                  <div className={`bg-white rounded-xl `}>
                    <img
                        src={process.env.NEXT_PUBLIC_STORAGE + vegetationData.image_thumbnail}
                        alt=""
                    ...
                    >
                      <div className={`w-full pr-3 px-6`}>
                        <div className={`flex gap-5 items-center mb-3`}>
                          ...
                          <h1 className={`text-gray-400`}>{vegetationData.createdAt}</h1>
                        </div>
                        ...
                        <div className={`text-gray-500 text-lg`}
                             dangerouslySetInnerHTML={{__html: vegetationData.desc}}/>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className={`flex flex-col gap-5`}>
                  {shuffle.map((vegetation) => (
                      <div>
                        <img
                            src={`${process.env.NEXT_PUBLIC_STORAGE}${vegetation.image_thumbnail}`}
                            alt="animals"
                        ...
                        <div>
                          ...
                          <div className={`text-gray-500 text-sm`}
                               dangerouslySetInnerHTML={{__html: vegetation.desc}}/>
                        </div>
                      </div>
