...
export default function page({params}) {
  ...
  const fetchData = async () => {
    ...
    const res = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/foods');
    setFoods(res.data.data);

    const res_data = await axios.get(process.env.NEXT_PUBLIC_API + '/cities/foods/' + params.id);
    const data = res_data.data.data;
    data.createdAt = new Date(data.created_at).toLocaleDateString('id-ID', options);
    setFoodData(data)
  }

  const shuffleData = async (foods) => {
    if (foods) {
      const shuffled = [...foods]; // Create a shallow copy of the foods array
              ...
              <div className={`w-full md:w-[80%] pr-0 md:pr-5`}>
                <div>
                  <div className={`bg-white rounded-xl `}>
                    <img
                        src={process.env.NEXT_PUBLIC_STORAGE + foodData.image_thumbnail}
                        alt=""
                    ...
                    >
                      <div className={`w-full pr-3 px-6`}>
                        <div className={`flex gap-5 items-center mb-3`}>
                          ...
                          <h1 className={`text-gray-400`}>{foodData.createdAt}</h1>
                        </div>
                        ...
                        <div className={`text-gray-500 text-lg`}
                             dangerouslySetInnerHTML={{__html: foodData.desc}}/>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className={`flex flex-col gap-5`}>
                  {shuffle.map((food) => (
                      <div>
                        <img
                            src={`${process.env.NEXT_PUBLIC_STORAGE}${food.image_thumbnail}`}
                            alt="animals"
                        ...
                        <div>
                          ...
                          <div className={`text-gray-500 text-sm`}
                               dangerouslySetInnerHTML={{__html: food.desc}}/>
                        </div>
                      </div>
