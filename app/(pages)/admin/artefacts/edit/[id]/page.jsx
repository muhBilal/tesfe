...
export default function Page({ params }) {
  ...
  const fetchData = async () => {
    try {
      const query = await axios.get(
        process.env.NEXT_PUBLIC_API + `/cities/artefacts/${params.id}`
      );
  ...
  const handleUpdate = async (data) => {
    ...
    try {
      await axios.post(
        process.env.NEXT_PUBLIC_API + `/cities/artefacts/update/${params.id}`,
        formData,
        {
          headers: {
                ...
                <div className="mb-6">
                  ...
                  >
                    {!dirtyFields.image_thumbnail ? (
                      <img
                        src={process.env.NEXT_PUBLIC_STORAGE + imageFileThumbnail}
                        alt="image"
                        className="w-full h-full object-cover"
                      />
                    ) : (
