...
const page = () => {
  ...
  const [artefactsDisplay, setArtefactsDisplay] = useState([]);

  const fetchData = async () => {
    try {
      const query = await axios.get(
        process.env.NEXT_PUBLIC_API + "/cities/artefacts"
      );
      const res = await query.data;
      console.log(res);
      setArtefacts(res.data);
      setArtefactsDisplay(res.data);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.post(
          process.env.NEXT_PUBLIC_API + "/cities/artefacts/delete/" + id
        );
        fetchData();
  ...
  const handleSearch = (keyword) => {
    const filteredItems = artefacts.filter((data) =>
      data.name.toLowerCase().includes(keyword.toLowerCase())
    );

    setArtefactsDisplay(filteredItems);
  };
        ...
        <div className="flex-none w-full max-w-full px-3">
          <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div className="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
              <h6 className="dark:text-white">Tabel Artefak</h6>
              <div className="flex items-center justify-between pb-4 pt-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Cari data artefak..."
                  />
                </div>
                <div>
                  <Link
                    href={"/admin/artefacts/create"}
            ...
            <div className="flex-auto px-0 pt-0 pb-2">
              <div className="p-0 overflow-x-auto">
                <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                  ...
                  <tbody>
                    {artefactsDisplay.map((artefact, index) => (
                      <tr>
                        ...
                        <td className="p-2 flex justify-center gap-3 bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <Link
                            href={`/admin/artefacts/edit/${artefact.id}`}
                            className={`btn-primary-lite`}
                          >
                            Edit
                          </Link>
                          <Modal label={`Lokasi`} data={artefact} />
                          <button
